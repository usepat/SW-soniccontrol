...
class DeviceDiscovery(abc.ABC):
    ...
    async def list_fw_device_names(self, include_ttys: bool = True, include_disks: bool = True) -> List[str]:
        return [device.display_name for device in await self.list_fw_device_infos(include_ttys, include_disks)]
