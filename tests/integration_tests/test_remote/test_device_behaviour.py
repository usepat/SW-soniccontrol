...
async def test_if_input_source_can_be_set_if_analog_is_set(remote_controller):
    answer = await remote_controller.send_command(commands.SetControlMode(ControlMode.ANALOG))
    assert_answer(answer, {EFieldName.CONTROL_MODE: ControlMode.ANALOG })
    answer = await remote_controller.send_command(commands.SetControlMode(ControlMode.REMOTE))
    assert_answer(answer, {EFieldName.CONTROL_MODE: ControlMode.REMOTE })
